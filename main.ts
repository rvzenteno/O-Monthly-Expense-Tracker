...
export default class MonthlyExpenseTrackerPlugin extends Plugin {
    ...
    updateStatusBar() {
        ...
        if (unpaid.length > 0) {
            this.statusBarItem.textContent = `💳 ${unpaid.length}/${total} unpaid`;
...
class DashboardView extends ItemView {
    ...
    renderExpenseItem(container: HTMLElement, expense: RecurringExpense) {
...
class AddExpenseModal extends Modal {
    ...
    constructor(app: App, plugin: MonthlyExpenseTrackerPlugin, expense: RecurringExpense | null) {
        super(app);
        this.plugin = plugin;
    ...
    onOpen() {
        ...
        new Setting(form)
            ...
            .addText(text => {
                text.setValue(this.expense?.name || '');
        ...
        new Setting(form)
            .setName('Amount')
            .addText(text => {
                text.setValue(this.expense?.amount.toString() || '');
        ...
        new Setting(form)
            ...
            .addText(text => {
                text.setValue(this.expense?.paymentMethod || '');
        ...
        new Setting(form)
            ...
            .addText(text => {
                text.setValue(this.expense?.category || '');
    ...
    async handleSubmit() {
        ...
        const dueDay = parseInt((document.getElementById('expense-due-day') as HTMLInputElement).value);
...
class MarkPaidModal extends Modal {
    ...
    constructor(app: App, plugin: MonthlyExpenseTrackerPlugin, expense: RecurringExpense, month: string, onComplete: () => void) {
        super(app);
        this.plugin = plugin;
        this.expense = expense;
        this.month = month;
